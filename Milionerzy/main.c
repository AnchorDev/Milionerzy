		...
		else if (ev.type == ALLEGRO_EVENT_MOUSE_BUTTON_DOWN)
		{
			if (ev.mouse.button == 1)
			{
				if (new_game.win | new_game.lose) {
					...
					if (new_game.crowdAvailable && !new_game.lose && !new_game.win)
					{
						Crowd();
					}
					...
					if (new_game.phoneAvailable && !new_game.lose && !new_game.win)
					{
						Phone();
					}
					...
					if (new_game.fiftyAvailable && !new_game.lose && !new_game.win)
					{
						Fifty();
